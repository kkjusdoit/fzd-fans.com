...
const matches_1 = require("../../data/matches");
const matchesData = matches_1.matches;
Component({
    data: {
        opponents: [],
        periods: [],
    ...
    methods: {
        ...
        initFilters() {
            ...
            const rawPartners = [...new Set(data.map(m => m.partner).filter((p) => !!p && p !== '——'))].sort();
        ...
        applyFilters() {
            ...
            const filtered = data.filter(m => {
                if (opponentFilter && m.opponent !== opponentFilter)
                    return false;
                if (periodFilter && m.period !== periodFilter)
                    return false;
                if (levelFilter && m.level !== levelFilter)
                    return false;
                if (partnerFilter && m.partner !== partnerFilter)
                    return false;
                if (resultFilter && m.result !== resultFilter)
                    return false;
                return true;
            });
